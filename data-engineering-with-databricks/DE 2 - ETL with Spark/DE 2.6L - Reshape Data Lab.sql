...
AS
SELECT * FROM (
  SELECT user_id, event_name, count(event_name) as event_cnt FROM EVENTS
  WHERE event_name in 
...
-- MAGIC
-- MAGIC event_names = ["cart", "pillows", "login", "main", "careers", "guest", "faq", "down", "warranty", "finalize", 
...
-- MAGIC     .filter(col("event_name").isin(event_names))
-- MAGIC     .groupBy("user")
